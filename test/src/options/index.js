...
document.querySelector('#add-user button').addEventListener(
  ...
  () => {
    ...
    postRequestToken(consumer_key, consumer_secret)
...
if (!!window.location.search) {
  ...
  chrome.storage.local.get(['tokens'], ({ tokens: { oauthToken, oauthTokenSecret } }) => {
    if (oauth_token !== oauthToken) {
      return
    }

    postAccessToken(consumer_key, consumer_secret, oauthToken, oauthTokenSecret, oauth_verifier)
      .then(({ accessToken, accessTokenSecret, userId, screenName }) => {
